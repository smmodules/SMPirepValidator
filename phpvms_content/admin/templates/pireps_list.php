...
elseif($pirep->accepted == PIREP_PENDING)
    echo '<span class="text-danger">Approval Pending</span>';
...
?>
    <table id="details_dialog_<?php echo $pirep->pirepid;?>"  style="display:none; border-left: 3px solid #FF6633; margin-top: 3px;padding-left: 3px;" width="100%">

        <!--Start SmartModules customization-->
        <?php
        // convert timestamp $submitdate to string
        $submitdate = gmdate("Ymd", $pirep->submitdate);
        $network = trim(SMPirepValidator::getStringBetween($pirep->log, "flying on ", "*"));

        // Get if Log IVAO
        $userId = PilotData::getFieldValue($pirep->pilotid, 'Ivao ID');
        $netivao = 'IVAO';
        $pilotdatalog = SMPirepValidatorData::findPilotDataLog($netivao,$userId, $pirep->code.$pirep->flightnum, $pirep->depicao, $pirep->arricao);
        if (empty($pilotdatalog)) {
            // Get if Log VATSIM
            $userId = PilotData ::getFieldValue($pirep -> pilotid, 'Vatsim ID');
            $netvatsim = 'VATSIM';
            $pilotdatalog = SMPirepValidatorData ::findPilotDataLog($netvatsim, $userId,$pirep -> code.$pirep -> flightnum, $pirep -> depicao, $pirep -> arricao);
            if (!empty($pilotdatalog)) {
                $network = 'VATSIM';
            } else {
                $network = 'N/A';
        ...
        <tr>
            <td colspan="2" style="background-color: #dedede">
                <strong>Network: </strong><i><?php echo $network;?></i>
            </td>
            <td colspan="3" style="background-color: #dedede">
                <strong>Found Log: </strong>
                <?php
                $userVID = PilotData::getFieldValue($pirep->pilotid, 'Ivao ID');
                $userCID = PilotData::getFieldValue($pirep->pilotid, 'Vatsim ID');
                if($pilotdatalog) {
                    echo '<span style="color: #126e12 "><i>YES </i> <a id="dialog" class="jqModal" style="font-weight: 400;" href="'.SITE_URL.'/admin/action.php/SMPirepValidatorAdmin/viewlog?logid='.$pilotdatalog->id.'"> View Log</a></span>';
                } else {
                    echo '<span style="color: #ff0000 "><i>NO</i></span>';
                    $flightnum = $pirep->code . $pirep->flightnum;
                    if (!empty($userVID) OR !empty($userCID)) {
                        echo '<span><strong>&nbsp;&nbsp;&nbsp;Manual Check: &nbsp;</strong></span>';
                    }
                    if (!empty($userVID)){
                        echo '&nbsp;<span><a target="_blank" href="https://tracker.ivao.aero/?userId='.$userVID.'&callsign='.$flightnum.'" style="text-decoration: none; color: #ff0000">IVAO VID-'.$userVID.' |  </a></span>';
                    }
                    if (!empty($userCID)) {
                        echo '&nbsp;<span><a target="_blank" href="https://stats.vatsim.net/stats/'.$userCID.'" style="text-decoration: none; color: #ff0000">VATSIM CID-'.$userCID.'</a></span>';
                    }
                }
                ?>
            </td>
        </tr>
        <!--End SmartModules customization-->
        ...
        <tr>
            ...
            <td><strong>Landing Rate: </strong>
                <?php
                if ($pirep -> landingrate < -500) {
                    echo '<span class="text-danger"><strong>'.$pirep -> landingrate.'</strong></span>';
                } else {
                    echo $pirep -> landingrate.'</span>';
                }
                ?>
            </td>
        </tr>
